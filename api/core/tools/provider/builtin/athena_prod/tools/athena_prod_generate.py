...
class AthenaGenerate(BuiltinTool):
    def _invoke(
        ...
        send_url = tool_parameters.get("send_url", "")
        ...
        if send_url:
            res = client.generate(app_id=app_id, query=query, history=history, kb_ids=kb_ids, is_multiple_project_query=is_multiple_project_query, is_qa_query=is_qa_query, send_url=send_url)
        else:
            res = client.generate(app_id=app_id, query=query, history=history, kb_ids=kb_ids, is_multiple_project_query=is_multiple_project_query, is_qa_query=is_qa_query)
